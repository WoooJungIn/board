...
//  마치 브라우저에서 URL을 요청한 것처럼 환경을 만들어준다.
    ...
    //  요청을 처리해주는 WebAppliationContext를 불러온다.
    @Autowired
...
//         가짜 MVC에 WebApplicationContext를 전달한 후 환경을 생성한다.
    ...
    public void testModify() throws Exception {
        String bno = mockMvc.perform(
                MockMvcRequestBuilders.post("/board/modify")
                        .param("title", "테스트 수정된 글 제목1")
                        .param("content", "테스트 수정된 내용1")
                        .param("writer", "hds1234")
                        .param("bno", "29")
        ).andReturn().getModelAndView().getModelMap().toString();
    ...
    public void testGoModify() throws Exception {
        String result = mockMvc.perform(MockMvcRequestBuilders.get("/board/modify").param("bno", "30"))
                .andReturn().getModelAndView().getModelMap().toString();
        log.info(result);
    }
}
